...
path = "/Users/maggiesun/Documents/Testing/pdftoimage2-2/" 
# put pdf file name
...
def pic_getinfo(doc,page, item): 
    ...
    rect = page.get_image_bbox(name) # bounding box of a image, left, top, width, height
#     pix = fitz.Pixmap(page)
    img = Image.open(io.BytesIO(doc.extractImage(xref)['image'])) # Image index number
...
#     print(image_list)
    ...
    if image_list:
         print("[+] Found a total of "+ str(len(image_list))+" images in page" + str(page_index+1))
    else:
...
#         print(text)
        for image_index, img in enumerate(page.get_images(), start=1):
            ...
            #set word Rectangle that we want to detect -> note, Make decisions by guessing
            ...
            # use 96 * 96 to filter the blank image
            if image.info['dpi'] != (96, 96):
#                 image.show()
                # save it to local disk
                # page.get_textbox(word_rect) is Desired output - the word detect in word Rectangle
                imgname = str(page.get_textbox(word_rect)) +'_' + pdf_file_name+'_' +str(page_index+1)+'_' +str(image_index)+'.'+ str(image_ext)
